...
def main() -> None:
    ...
    parser.add_argument("--method", action="append", default=None, help="Optional method override. Repeat to run multiple methods.")
    ...
    methods = list(args.method or resolved_specs["methods"])
    unknown = sorted(set(methods) - set(resolved_specs["methods"]))
    if unknown:
        raise ValueError(
            "Method override contains methods that are not in the report config: "
            + ", ".join(unknown)
        )
