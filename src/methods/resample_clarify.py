"""Intent stabilization via all-stage resampling with clarification fallback."""
...
def _apply_resample_ablation_defaults(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if _ablation_enabled(updated, "no_selective_resample") or _ablation_enabled(updated, "resample_all_stages"):
        rs_cfg["repair_policy"] = "all_stages"
        rs_cfg["selective_repair_enabled"] = False
...
def run_resample_clarify(
    ...
    *,
    ...
    repair_policy = str(rs_cfg.get("repair_policy", "all_stages")).lower()
    ...
    while True:
        ...
        current_round += 1
        resample_all = (
            repair_policy == "all_stages"
            or not selective_repair_enabled
